...
SELECT b.buyer_name AS person_name, b.buyer_contact_no AS contact_no,
'Buyer' AS role
FROM Buyer AS b
JOIN Village AS v
...
UNION
SELECT l.landlord_name AS person_name, l.landlord_contact_no AS contact_no,
'Landlord' AS role
FROM Landlord AS l
JOIN Village AS v
...
WHERE v.village_name = 'Kakori'
UNION
SELECT lb.labour_name AS person_name, lb.contact_no AS contact_no, 'Labour'
AS role
FROM Labour AS lb
JOIN Village AS v
...
D.district_name,
S.state_name,
(SELECT COUNT(*) FROM Tenant T WHERE T.village_no = V.village_no and
T.district_no=V.district_no and T.state_no=V.state_no) AS tenant_count,
(SELECT COUNT(*) FROM Landlord L WHERE L.village_no = V.village_no and
L.district_no=V.district_no and L.state_no=V.state_no) AS landlord_count
FROM
Village V
JOIN
...
6. There is a need for laborers in the village ‘Kakori’. Find laborers who are not
hired and can grow crops that are grown in the village ‘Kakori’. Also, laborers
should not be from the village ‘Kakori’.
...
7. Find laborers who can handle machinery and can work for 28 hours per
week in the village ‘Kakori’
...
SUM(COALESCE(THL.work_hour_per_week, 0)) AS
work_hours_under_tenant,
...
SUM(COALESCE(LHL.work_hour_per_week, 0)) AS
work_hours_under_landlord,
...
8. Let’s say a farmer wants to grow ‘Cotton’ on his land but if the district wise
production of ‘Rice’ has exceeded 70% of its target then list all the crops that
can be grown on his land along with its percentage of target achieved district
wise in order to avoid overproduction of a particular crop.
...
9. Find the landlord name and contact number who owns land in more than
one village.
...
FROM Buyer b
JOIN Village v on b.village_no = v.village_no and b.district_no = v.district_no and
b.state_no=v.state_no
...
11. Give the landlord name, number and address of land which satisfies a
buyer’s requirement.
...
12. Top 3 Most Popular Crop Requirements among Buyers across all the
districts of the state ‘Uttar Pradesh’.
...
13. Give contribution of different villages of the districts of ’Uttar Pradesh’ in a
production of a crop say , ‘Rice’.
  
SELECT vp.village_no, vp.district_no, vp.state_no, vp.crop_id, v.village_name AS
village,
...
14. Find all crops produced in the village Kakori of district lucknow and state
uttar pradesh, with their respective quantity produced.
  
select village_name, district_name, state_name, crop_id,vp.quantity from
village_production as vp
...
where state_name = 'Uttar Pradesh' and district_name='Lucknow' and
village_name='Kakori'
...
16. Find buyer who are interested in buying land of village(04,03,01) with
landlord contact given as 9991234601
...
17. Landlord has a fertilizer urea and he wants to know whether it can be used
for other crops that can be grown on his land. If yes then return crop names.
...
19. Find percentage of target that has been achieved by each crop of district
(01,01) ,i.e, display (produced*100)/target for each crop of a given district.
...
20. I want to know the number of laborers involved in production of each crop
grown in a district.
...
LEFT JOIN
landlord_hires_labour AS lhl ON lnd.landlord = lhl.head_farmer AND
lnd.temporary_landlord IS NULL
LEFT JOIN
...
21. Find fertilizers that can be used for each crop that can be grown on ‘clay’
soil
